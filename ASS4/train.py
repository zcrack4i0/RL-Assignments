...
def train_and_validate(model_type, env_name, config):
    ...
    wandb.init(project="CMPS458-Assignment3-Mostafa", config=config, reinit=True, name=run_name, group="optimization",mode="online")
...
def test_best_model(model_type, env_name, config, model_path):
    ...
    wandb.init(project="CMPS458-Assignment3-mostafa-test", config=config, reinit=True, name=run_name, group="final_testing",mode="online")
...
if __name__ == "__main__":
    ...
    # ==========================
    try:
        with open("best_configs/final_best_registry.json", "r") as f:
            best_registry = json.load(f)
        print("Loaded best registry from best_configs/final_best_registry.json")
    except Exception as e:
        print(f"Warning: Could not load best registry JSON: {e}. Using in-memory best_registry if available.")
